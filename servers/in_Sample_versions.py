...
def in_sample_verions(input: Inputs, output: Outputs, session: Session):
    ...
    def ok_verions():
        ...
        print(add)
        parameter_versions  = get_project_versions_param(global_session.get_id_proyecto())
        print(parameter_versions )
        list.set({
        version['param_version_name']: version['param_version_name']
        for version in parameter_versions
            } if parameter_versions else {"": "No hay versiones de parámetros"})
        #print(list.get)
        ui.update_select("version_selector",choices=list.get())# 
